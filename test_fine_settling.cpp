...
void Report(LBM::Domain &dom, void *UD)
{
    ...
    {
        String fs;
        fs.Printf("%s.out","settling_hex_10.0");
        
        dat.oss_ss.open(fs.CStr(),std::ios::out);
        dat.oss_ss<<Util::_10_6<<"Time"<<Util::_8s<<"Re"<<Util::_8s<<"Vx"<<Util::_8s<<"Vy"<<Util::_8s<<"X"<<"Y\n";
    }else{
        double v = dom.Particles.back().V(1);
        dat.oss_ss<<Util::_10_6<<dom.Time<<Util::_8s<<v*dat.R*2.0/dat.nu<<Util::_8s<<dom.Particles.back().V(0)<<dom.Particles.back().V(1)<<<<dom.Particles.back().X(0)<<dom.Particles.back().X(1)<<std::endl;
...
int main (int argc, char **argv) try
{
    ...
    dom.modexy = 0;
    //solving
    dom.Solve( Tf, dtout, "test_fine_settling", NULL, Report);
